...
const ComponentsPanel: React.FC<{
   ...
   const findComponentIdForSource = (beforeIds: Set<string>, source: string, previousSourceToId: Map<string, string>): string | null => {
   ...
   const loadConfiguredComponents = async (showNotification = false) => {
      ...
      for (const storedSource of config.sources) {
         ...
         try {
            ...
            Notification.warning(`Could not register configured component source: ${storedSource}. ${String(error?.message ?? error)}`, {
               autoClose: 5000,
            });
         }
      }
   ...
   const onCheck: TreeProps["onCheck"] = (checkedKeysValue) => {
      const keys = Array.isArray(checkedKeysValue) ? checkedKeysValue : (checkedKeysValue.checked as React.Key[]);

      const nextCheckedComponentIds = new Set(
         keys.map((value) => String(value)).filter((value) => componentCatalog.some((component) => component._id === value)),
      );
      ...
      >
         <div className="amphi-ComponentsPanel">
            ...
            <div className="amphi-ComponentsPanel__actions">
               <Button type="primary" icon={<PlusOutlined />} className="amphi-ComponentsPanel__actionButton" onClick={openModal}>
                  {/* Add Component */}
                  添加组件
               </Button>
               <Button
            ...
            <div className="amphi-ComponentsPanel__tree">
               {componentCatalog.length === 0 ? (
                  ...
                  <Tree checkable defaultExpandAll checkedKeys={Array.from(registeredIds)} onCheck={onCheck} treeData={treeData} />
               )}
            </div>

            <div className="amphi-ComponentsPanel__footer">Browse components and enable/disable components.</div>
            ...
            <Modal
               ...
               destroyOnClose
            >
               <Radio.Group value={sourceMode} onChange={(event) => setSourceMode(event.target.value)} style={{ marginBottom: 12 }}>
