...
class SchedulerAPI {
   ...
   static runJob(id: string): Promise<{ success: boolean; output?: string; error?: string }> {
...
const JobForm: React.FC<{
   ...
   const [scheduleType, setScheduleType] = useState<"date" | "interval" | "cron" | "trigger">(initialValues?.schedule_type || "date");
   const [dateType, setDateType] = useState<"once" | "daily" | "weekly" | "monthly" | "every_x_days">(initialValues?.date_type || "once");
   ...
   return (
      ...
      >
         ...
         >
            ...
            <Form.Item style={{ marginBottom: 16 }} /* spacing */ label="管道路径" required>
               <Space.Compact style={{ width: "100%" }}>
                  <Form.Item name="pipeline_path" noStyle rules={[{ required: true, message: "请选择一个管道文件" }]}>
            ...
            <Form.Item style={{ marginBottom: 16 }} name="schedule_type" label="计划类型">
            ...
            {scheduleType === "date" && (
               <>
                  <Form.Item style={{ marginBottom: 16 }} name="date_type" label="日期类型">
                  ...
                  {dateType !== "every_x_days" ? (
                     <Form.Item
                        style={{ marginBottom: 16 }}
                        name="run_date"
                        label={dateType === "once" ? "运行日期与时间" : "开始日期和时间"}
...
const getPythonCode = async (path: string, commands: JupyterFrontEnd["commands"], docManager: IDocumentManager): Promise<string> => {
...
const SchedulerPanel: React.FC<SchedulerPanelProps> = ({ commands, docManager }) => {
   ...
   const handleDeleteJob = (jobId: string) => {
      ...
      Notification.promise(promise, {
         ...
         error: {
            message: (err: unknown) => `Failed to delete job: ${err instanceof Error ? err.message : String(err)}`,
         },
      });
   ...
   const handleRunJob = async (jobId: string) => {
      ...
      Notification.promise(promise, {
         ...
         error: {
            message: (e: unknown) => `Pipeline execution failed: ${e instanceof Error ? e.message : String(e)}`,
         },
      });
   };
   ...
   const handleDeleteRun = (runId: number) => {
      ...
      Notification.promise(promise, {
         ...
         error: {
            message: (err: unknown) => `Failed to remove monitoring entry: ${err instanceof Error ? err.message : String(err)}`,
         },
      });
   };
   ...
   const handleClearRuns = () => {
      ...
      Notification.promise(promise, {
         ...
         error: {
            message: (err: unknown) => `Failed to clear monitoring entries: ${err instanceof Error ? err.message : String(err)}`,
         },
      });
   };
                                          ...
                                          <List.Item
                                             ...
                                             actions={[
                                                <Tooltip title="Run now" key="run">
                                                   <Button type="text" icon={<PlayCircleOutlined />} onClick={() => handleRunJob(job.id)} />
                                                </Tooltip>,
                                                <Tooltip title="Edit" key="edit">
                                                   <Button type="text" icon={<EditOutlined />} onClick={() => handleEditJob(job)} />
                                                </Tooltip>,
                        ...
                        children: (
                           <>
                              ...
                              {monitoringLoading ? (
                                 ...
                                 <Empty description="No runs recorded yet" image={Empty.PRESENTED_IMAGE_SIMPLE} />
                              ) : (
                                 <List
            ...
            >
               {logsLoading ? (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                     <div>
                        <strong>Status:</strong> {selectedRun?.status === "success" ? "Succeeded" : "Failed"} at{" "}
   ...
   activate: (app: JupyterFrontEnd, palette: ICommandPalette, docManager: IDocumentManager) => {
      ...
      commands.addCommand(CommandIDs.open, {
         ...
         execute: () => {
            ...
            if (ENABLE_SCHEDULER && !widget.isAttached) shell.add(widget, "left");
