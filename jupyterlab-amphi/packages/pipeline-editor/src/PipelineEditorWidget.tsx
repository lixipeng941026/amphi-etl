...
import { ABCWidgetFactory, Context, DocumentRegistry, DocumentWidget } from "@jupyterlab/docregistry";
...
const PipelineWrapper: React.FC<IProps> = ({
   ...
   function PipelineFlow(context) {
                           ...
                           ? {
                                ...
                                data: {
                                   ...
                                   backend: {
                                      ...node.data.backend,
                                      engine:
                                         node.data?.backend?.prefix && node.data.backend.prefix !== sourceBackend.prefix
      ...
      function generateUniqueNodeName(type: string, nodes: any): string {
         ...
         const existingNodesOfType = nodes.filter((node) => node.type === type && node.data?.nameId);
         ...
         (location?: { x: number; y: number }) => {
            ...
            if (reactFlowInstance && location) {
               ...
               Array.from(fileBrowser.selectedItems()).forEach(async (item: any) => {
                  ...
                  const { id: nodeType, default: nodeDefaults } = PipelineService.getComponentIdForFileExtension(
                     fileExtension,
                     componentService,
         ...
         >
            <ReactFlowProvider>
               {enableHorizontalPalette ? (
                  ...
                  >
                     <ComponentPalette componentService={componentService} onRefreshed={handleComponentsRefresh} />
...
export class PipelineEditorFactory extends ABCWidgetFactory<DocumentWidget> {
   ...
   protected createNewWidget(context: DocumentRegistry.Context): DocumentWidget {
      ...
      async function showCodeModal(code: string, commands: any, componentService: any, isDagsterCode: boolean) {
         ...
         function CodeModal() {
            ...
            const handleExportToDagster = async () => {
               try {
                  const dagsterCode = CodeGeneratorDagster.generateDagsterCode(context.model.toString(), commands, componentService, true);
                     ...
                     >
                        ...
                        >
                           ...
                           <div style={{ position: "absolute", top: 0, right: 25 }}>
                              ...
                              >
                                 {copyStatus === "copied" ? "Copied!" : copyStatus === "loading" ? "Loading..." : "Copy to clipboard"}
                              </Dropdown.Button>
                           </div>
                        </div>
                     </Modal>
                  ...
                  >
                     ...
                     <div style={{ marginTop: "16px", fontSize: "13px", color: "#666" }}>
                        {executionMode === "full" ? (
                           ...
                           <div>
                              <strong>Step-by-Step:</strong> Each component executes one by one and shows results in the console. Easier to
    ...
    */
      ...
      const kernelName = Toolbar.createKernelNameItem(props.context.sessionContext);
